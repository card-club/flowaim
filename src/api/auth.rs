...
pub fn get_acces_token(configsettings: &HashMap<String, String>) -> Result<String, ureq::Error> {
    let private_key = configsettings.get("private_key").unwrap();
    let public_key = configsettings.get("public_key").unwrap();
    let user_id = configsettings.get("user_id").unwrap();

    let AuthCodeResponse { auth_code } =
        ureq::post(format!("{}/v1/auth/code", configsettings.get("api_url").unwrap()).as_str())
            .send_json(ureq::json!({ "userId": user_id }))?
            .into_json()?;

    let signature = sign_message(&auth_code, public_key, private_key);

    let AuthTokenResponse { access_token } =
        ureq::post(format!("{}/v1/auth/token", configsettings.get("api_url").unwrap()).as_str())
            .send_json(ureq::json!({
                "userId": user_id,
                "authCode": auth_code,
                "signature": signature,
